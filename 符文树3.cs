...
namespace 符文树3 {
    public static partial class LocalStorage {
        ...
        public static GameObject 两点间连线(Vector3 起始点, Vector3 末点) {
            GameObject A = new GameObject();
            A.transform.SetParent(游戏物体池["正文区"].transform);
            A.AddComponent<Image>();
        ...
        public static void 创建滚动列表(GameObject 父物体,string Y,string Name) {
            GameObject A = 父物体.创建矩形(Y);
            游戏物体池.Add(Name, A.创建垂直布局滚动条());
        }
...
namespace 符文树3 {
    public static partial class LocalStorage {
        ...
        public static 正文区 正文区;
        public static 快捷按钮目录 快捷按钮目录;
    ...
    public class 存档类 {
        ...
        public Dictionary<string, 快捷按钮数据> 已创建的快捷按钮 = new Dictionary<string, 快捷按钮数据>();
...
namespace 符文树3 {
    ...
    public class 快捷按钮目录 { 
    public 快捷按钮目录() {
            GameObject A = 控制台.终端.创建矩形("0 0 110 100%");
            A.AddComponent<Image>().color = new Color(195 / 255f, 134 / 255f, 134 / 255f, 255 / 255f);
            创建滚动列表(A, "0 0 100% 100%","快捷按钮列表");
            刷新();
        }
        public void 刷新() {
            foreach (var ins2 in 存档.已创建的快捷按钮) {
                Destroy(游戏物体池[ins2.Key]);
                游戏物体池.Remove(ins2.Key);
            }
            foreach (var ins in 存档.已创建的快捷按钮) {
                GameObject A = 游戏物体池["快捷按钮列表"].创建矩形("0 0 90 80");
                Debug.Log("KJ");
                A.SetText(ins.Key);
                A.AddComponent<Image>().color = new Color(255 / 255f, 255 / 255f, 255 / 255f, 255 / 255f);
                游戏物体池.Add(ins.Key, A);
                A.OnClick(() => {
                    Debug.Log("KJ3："+ ins.Value.事件);
                    控制台.执行(ins.Value.事件);
                });
            }
        }
    ...
    public class 节点 {
        ...
        public 节点(string ID1) {
            ...
            GameObject A = 游戏物体池["正文区"].创建矩形("50% 50% 300 100");//这是节点背景
    ...
    public class 笔记本 {
        ...
        public 笔记本(string id) {
            ID = id;
            GameObject A = MainPanel.创建矩形("20% 10% 55% 80%");
            A.AddComponent<Image>().color = new Color(195 / 255f, 134 / 255f, 134 / 255f, 255 / 255f);
    ...
    public class 正文区 {
        public 正文区() {
            GameObject A = MainPanel.创建矩形("20% 10% 55% 80%");
            A.name = "ScrollRect";
            GameObject B = A.创建矩形("0 0 100% 100%");
            B.name = "Viewport";
            GameObject C = B.创建矩形("0 0 2000 2000");
            C.name = "Content";
            A.右键菜单("创建符文树节点", () => {
                控制台.执行("创建符文树节点");
            });
            A.AddComponent<ScrollRect>();
            B.AddComponent<RectMask2D>();
            A.GetComponent<ScrollRect>().viewport = B.GetComponent<RectTransform>();
            A.GetComponent<ScrollRect>().content = C.GetComponent<RectTransform>();
            C.AddComponent<Image>().color = new Color(195 / 255f, 134 / 255f, 134 / 255f, 255 / 255f);
            游戏物体池.Add("正文区", C);
        }
    }
}
        ...
        public static void 初始化控制台指令() {
            ...
            控制台["撤销正在进行的连线"] = t => {
                if (正在连线.当前正在连线) {
                    ...
                    正在连线.当前正在连线 = false;
                    存档.符文树连线ID--;
                    存档.已创建的符文树[存档.当前符文树].符文树连线.Remove(正在连线.不定连线);
                    Debug.Log(正在连线.不定连线.ID);
                    Destroy(游戏物体池[正在连线.不定连线.ID]);
                    游戏物体池.Remove(正在连线.不定连线.ID);
            ...
            控制台["创造快捷按钮"] = t => {
                存档.快捷按钮ID++;
                Debug.Log("KID:"+ 存档.快捷按钮ID);
               存档.已创建的快捷按钮.Add("快捷按钮" + 存档.快捷按钮ID,new 快捷按钮数据() {ID="快捷按钮" + 存档.快捷按钮ID,事件 = ""});
                快捷按钮目录.刷新();
            };
            控制台["快捷按钮设置"] = t => {
                Debug.Log("KJ2");
                #region t.校验();
                if (t.Length != 3) {
                    Debug.Log("无法识别：" + t + "格式不正确");
                    return;
                }
                if (!存档.已创建的快捷按钮.ContainsKey(t[1])) {
                    Debug.Log("不存在此按钮：" + t[1]);
                    return;
                }
                if (!控制台.ContainsKey(t[2])) {
                    Debug.Log("不存在此指令：" + t[2]);
                    return;
                }
                #endregion
                Debug.Log("KJ5");
                存档.已创建的快捷按钮[t[1]].事件 = t[2];
                };//快捷按钮设置 快捷按钮1 创造快捷按钮
        }
    }
}
